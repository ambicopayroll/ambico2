...
$t_tgl_2017_grid = NULL; // Initialize page object first

class ct_tgl_2017_grid extends ct_tgl_2017 {
	...
	// Page object name
	var $PageObjName = 't_tgl_2017_grid';

	// Grid form hidden field names
	var $FormName = 'ft_tgl_2017grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_tgl_2017"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->tgl_id->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->tgl_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->tgl_id->CurrentValue = strval($arKeys[0]); // tgl_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "t_jdkr_peg") {
				$this->tgl_id->CurrentValue = $this->tgl_id->getSessionValue();
			}
		$conn = &$this->Connection();
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_jdkr_peg") {
			$this->tgl_id->Visible = FALSE;
			if ($GLOBALS["t_jdkr_peg"]->EventCancelled) $this->EventCancelled = TRUE;
		}
