...
//
// Table class for t_tgl_2017
//
class ct_tgl_2017 extends cTable {
	var $tgl_id;
	var $tgl;
	...
	function __construct() {
		...
		$this->TableVar = 't_tgl_2017';
		$this->TableName = 't_tgl_2017';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_tgl_2017`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// tgl_id
		$this->tgl_id = new cField('t_tgl_2017', 't_tgl_2017', 'x_tgl_id', 'tgl_id', '`tgl_id`', '`tgl_id`', 3, -1, FALSE, '`tgl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// tgl
		$this->tgl = new cField('t_tgl_2017', 't_tgl_2017', 'x_tgl', 'tgl', '`tgl`', ew_CastDateFieldForLike('`tgl`', 0, "DB"), 133, 0, FALSE, '`tgl`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_tgl_2017`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('tgl_id', $rs))
				ew_AddFilter($where, ew_QuotedName('tgl_id', $this->DBID) . '=' . ew_QuotedValue($rs['tgl_id'], $this->tgl_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->tgl_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@tgl_id@", ew_AdjustSql($this->tgl_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_tgl_2017view.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_tgl_2017view.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_tgl_2017add.php?" . $this->UrlParm($parm);
		else
			$url = "t_tgl_2017add.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_tgl_2017edit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_tgl_2017add.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_tgl_2017delete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "tgl_id:" . ew_VarToJson($this->tgl_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->tgl_id->CurrentValue)) {
			$sUrl .= "tgl_id=" . urlencode($this->tgl_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["tgl_id"]))
				$arKeys[] = ew_StripSlashes($_POST["tgl_id"]);
			elseif (isset($_GET["tgl_id"]))
				$arKeys[] = ew_StripSlashes($_GET["tgl_id"]);
			else
