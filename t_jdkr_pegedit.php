...
$t_jdkr_peg_edit = NULL; // Initialize page object first

class ct_jdkr_peg_edit extends ct_jdkr_peg {
	...
	// Page object name
	var $PageObjName = 't_jdkr_peg_edit';
	...
	function Page_Init() {
		...
		if (!$Security->CanEdit()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("t_jdkr_peglist.php"));
			else
	...
	function Page_Main() {
		...
		if (@$_GET["jdkr_id"] <> "") {
			...
			$this->RecKey["jdkr_id"] = $this->jdkr_id->QueryStringValue;
		} else {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("t_jdkr_peglist.php"); // Return to list page
			...
			while (!$this->Recordset->EOF) {
				if (strval($this->jdkr_id->CurrentValue) == strval($this->Recordset->fields('jdkr_id'))) {
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "t_jdkr_peglist.php")
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->jdkr_id->FldIsDetailKey)
			$this->jdkr_id->setFormValue($objForm->GetValue("x_jdkr_id"));
		if (!$this->pegawai_id->FldIsDetailKey) {
			$this->pegawai_id->setFormValue($objForm->GetValue("x_pegawai_id"));
		}
		if (!$this->tgl_id->FldIsDetailKey) {
			$this->tgl_id->setFormValue($objForm->GetValue("x_tgl_id"));
		}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->jdkr_id->CurrentValue = $this->jdkr_id->FormValue;
		$this->pegawai_id->CurrentValue = $this->pegawai_id->FormValue;
		$this->tgl_id->CurrentValue = $this->tgl_id->FormValue;
	...
	function RenderRow() {
		...
		$this->jk_id->ViewCustomAttributes = "";
			...
			$this->tgl_id->EditValue = ew_HtmlEncode($this->tgl_id->CurrentValue);
			if (strval($this->tgl_id->CurrentValue) <> "") {
				$sFilterWrk = "`tgl_id`" . ew_SearchString("=", $this->tgl_id->CurrentValue, EW_DATATYPE_NUMBER, "");
			$sSqlWrk = "SELECT `tgl_id`, `tgl` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `t_tgl_2017`";
			$sWhereWrk = "";
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				...
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$arwrk = array();
					$arwrk[1] = ew_HtmlEncode(ew_FormatDateTime($rswrk->fields('DispFld'), 0));
					$this->tgl_id->EditValue = $this->tgl_id->DisplayValue($arwrk);
					$rswrk->Close();
				} else {
					$this->tgl_id->EditValue = ew_HtmlEncode($this->tgl_id->CurrentValue);
				}
			} else {
				$this->tgl_id->EditValue = NULL;
			}
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				...
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$arwrk = array();
					$arwrk[1] = ew_HtmlEncode($rswrk->fields('DispFld'));
					$this->jk_id->EditValue = $this->jk_id->DisplayValue($arwrk);
					$rswrk->Close();
				} else {
					$this->jk_id->EditValue = ew_HtmlEncode($this->jk_id->CurrentValue);
				}
			} else {
				$this->jk_id->EditValue = NULL;
			}
	...
	function ValidateForm() {
		...
		if (!$this->tgl_id->FldIsDetailKey && !is_null($this->tgl_id->FormValue) && $this->tgl_id->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->tgl_id->FldCaption(), $this->tgl_id->ReqErrMsg));
		}
		...
		if (in_array("t_tgl_2017", $DetailTblVar) && $GLOBALS["t_tgl_2017"]->DetailEdit) {
			if (!isset($GLOBALS["t_tgl_2017_grid"])) $GLOBALS["t_tgl_2017_grid"] = new ct_tgl_2017_grid(); // get detail page object
			$GLOBALS["t_tgl_2017_grid"]->ValidateGridForm();
		}
		if (in_array("t_jk", $DetailTblVar) && $GLOBALS["t_jk"]->DetailEdit) {
			if (!isset($GLOBALS["t_jk_grid"])) $GLOBALS["t_jk_grid"] = new ct_jk_grid(); // get detail page object
			$GLOBALS["t_jk_grid"]->ValidateGridForm();
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// tgl_id
			$this->tgl_id->SetDbValueDef($rsnew, $this->tgl_id->CurrentValue, 0, $this->tgl_id->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("t_tgl_2017", $DetailTblVar) && $GLOBALS["t_tgl_2017"]->DetailEdit) {
						if (!isset($GLOBALS["t_tgl_2017_grid"])) $GLOBALS["t_tgl_2017_grid"] = new ct_tgl_2017_grid(); // Get detail page object
						$Security->LoadCurrentUserLevel($this->ProjectID . "t_tgl_2017"); // Load user level of detail table
						$EditRow = $GLOBALS["t_tgl_2017_grid"]->GridUpdate();
				...
				if ($EditRow) {
					if (in_array("t_jk", $DetailTblVar) && $GLOBALS["t_jk"]->DetailEdit) {
						if (!isset($GLOBALS["t_jk_grid"])) $GLOBALS["t_jk_grid"] = new ct_jk_grid(); // Get detail page object
						$Security->LoadCurrentUserLevel($this->ProjectID . "t_jk"); // Load user level of detail table
						$EditRow = $GLOBALS["t_jk_grid"]->GridUpdate();
						$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("t_tgl_2017", $DetailTblVar)) {
				...
				if ($GLOBALS["t_tgl_2017_grid"]->DetailEdit) {
					$GLOBALS["t_tgl_2017_grid"]->CurrentMode = "edit";
					$GLOBALS["t_tgl_2017_grid"]->CurrentAction = "gridedit";
			...
			if (in_array("t_jk", $DetailTblVar)) {
				...
				if ($GLOBALS["t_jk_grid"]->DetailEdit) {
					$GLOBALS["t_jk_grid"]->CurrentMode = "edit";
					$GLOBALS["t_jk_grid"]->CurrentAction = "gridedit";
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("t_jdkr_peglist.php"), "", $this->TableVar, TRUE);
		$PageId = "edit";
...
// Create page object
if (!isset($t_jdkr_peg_edit)) $t_jdkr_peg_edit = new ct_jdkr_peg_edit();

// Page init
$t_jdkr_peg_edit->Page_Init();

// Page main
$t_jdkr_peg_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_jdkr_peg_edit->Page_Render();
?>
...
var CurrentForm = ft_jdkr_pegedit = new ew_Form("ft_jdkr_pegedit", "edit");

// Validate form
ft_jdkr_pegedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			elm = this.GetElements("x" + infix + "_tgl_id");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $t_jdkr_peg->tgl_id->FldCaption(), $t_jdkr_peg->tgl_id->ReqErrMsg)) ?>");
...
ft_jdkr_pegedit.Form_CustomValidate = 
...
ft_jdkr_pegedit.ValidateRequired = true;
<?php } else { ?>
ft_jdkr_pegedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_jdkr_pegedit.Lists["x_pegawai_id"] = {"LinkField":"x_pegawai_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pegawai_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"pegawai"};
ft_jdkr_pegedit.Lists["x_tgl_id"] = {"LinkField":"x_tgl_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_tgl","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_tgl_2017"};
ft_jdkr_pegedit.Lists["x_jk_id"] = {"LinkField":"x_jk_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_jk_nm","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t_jk"};
...
<?php if (!$t_jdkr_peg_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t_jdkr_peg_edit->ShowPageHeader(); ?>
<?php
$t_jdkr_peg_edit->ShowMessage();
?>
<?php if (!$t_jdkr_peg_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t_jdkr_peg_edit->Pager)) $t_jdkr_peg_edit->Pager = new cPrevNextPager($t_jdkr_peg_edit->StartRec, $t_jdkr_peg_edit->DisplayRecs, $t_jdkr_peg_edit->TotalRecs) ?>
<?php if ($t_jdkr_peg_edit->Pager->RecordCount > 0 && $t_jdkr_peg_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t_jdkr_peg_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t_jdkr_peg_edit->PageUrl() ?>start=<?php echo $t_jdkr_peg_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t_jdkr_peg_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t_jdkr_peg_edit->PageUrl() ?>start=<?php echo $t_jdkr_peg_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t_jdkr_peg_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t_jdkr_peg_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t_jdkr_peg_edit->PageUrl() ?>start=<?php echo $t_jdkr_peg_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t_jdkr_peg_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t_jdkr_peg_edit->PageUrl() ?>start=<?php echo $t_jdkr_peg_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t_jdkr_peg_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="ft_jdkr_pegedit" id="ft_jdkr_pegedit" class="<?php echo $t_jdkr_peg_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_jdkr_peg_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_jdkr_peg_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if ($t_jdkr_peg->getCurrentMasterTable() == "pegawai") { ?>
...
<?php if ($t_jdkr_peg->jdkr_id->Visible) { // jdkr_id ?>
	<div id="r_jdkr_id" class="form-group">
		<label id="elh_t_jdkr_peg_jdkr_id" class="col-sm-2 control-label ewLabel"><?php echo $t_jdkr_peg->jdkr_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $t_jdkr_peg->jdkr_id->CellAttributes() ?>>
...
<input type="hidden" data-table="t_jdkr_peg" data-field="x_jdkr_id" name="x_jdkr_id" id="x_jdkr_id" value="<?php echo ew_HtmlEncode($t_jdkr_peg->jdkr_id->CurrentValue) ?>">
<?php echo $t_jdkr_peg->jdkr_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($t_jdkr_peg->pegawai_id->Visible) { // pegawai_id ?>
	<div id="r_pegawai_id" class="form-group">
		<label id="elh_t_jdkr_peg_pegawai_id" class="col-sm-2 control-label ewLabel"><?php echo $t_jdkr_peg->pegawai_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t_jdkr_peg->pegawai_id->CellAttributes() ?>>
...
<input type="hidden" id="x_pegawai_id" name="x_pegawai_id" value="<?php echo ew_HtmlEncode($t_jdkr_peg->pegawai_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_t_jdkr_peg_pegawai_id">
<?php
...
<input type="hidden" data-table="t_jdkr_peg" data-field="x_pegawai_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $t_jdkr_peg->pegawai_id->DisplayValueSeparatorAttribute() ?>" name="x_pegawai_id" id="x_pegawai_id" value="<?php echo ew_HtmlEncode($t_jdkr_peg->pegawai_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x_pegawai_id" id="q_x_pegawai_id" value="<?php echo $t_jdkr_peg->pegawai_id->LookupFilterQuery(true) ?>">
<script type="text/javascript">
ft_jdkr_pegedit.CreateAutoSuggest({"id":"x_pegawai_id","forceSelect":true});
</script>
...
<input type="hidden" name="s_x_pegawai_id" id="s_x_pegawai_id" value="<?php echo $t_jdkr_peg->pegawai_id->LookupFilterQuery(false) ?>">
</span>
<?php } ?>
<?php echo $t_jdkr_peg->pegawai_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($t_jdkr_peg->tgl_id->Visible) { // tgl_id ?>
	<div id="r_tgl_id" class="form-group">
		<label id="elh_t_jdkr_peg_tgl_id" class="col-sm-2 control-label ewLabel"><?php echo $t_jdkr_peg->tgl_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t_jdkr_peg->tgl_id->CellAttributes() ?>>
...
<input type="hidden" data-table="t_jdkr_peg" data-field="x_tgl_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $t_jdkr_peg->tgl_id->DisplayValueSeparatorAttribute() ?>" name="x_tgl_id" id="x_tgl_id" value="<?php echo ew_HtmlEncode($t_jdkr_peg->tgl_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x_tgl_id" id="q_x_tgl_id" value="<?php echo $t_jdkr_peg->tgl_id->LookupFilterQuery(true) ?>">
<script type="text/javascript">
ft_jdkr_pegedit.CreateAutoSuggest({"id":"x_tgl_id","forceSelect":true});
</script>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($t_jdkr_peg->tgl_id->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_tgl_id',m:0,n:10,srch:false});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" name="s_x_tgl_id" id="s_x_tgl_id" value="<?php echo $t_jdkr_peg->tgl_id->LookupFilterQuery(false) ?>">
</span>
<?php echo $t_jdkr_peg->tgl_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($t_jdkr_peg->jk_id->Visible) { // jk_id ?>
	<div id="r_jk_id" class="form-group">
		<label id="elh_t_jdkr_peg_jk_id" class="col-sm-2 control-label ewLabel"><?php echo $t_jdkr_peg->jk_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t_jdkr_peg->jk_id->CellAttributes() ?>>
...
<input type="hidden" data-table="t_jdkr_peg" data-field="x_jk_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $t_jdkr_peg->jk_id->DisplayValueSeparatorAttribute() ?>" name="x_jk_id" id="x_jk_id" value="<?php echo ew_HtmlEncode($t_jdkr_peg->jk_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x_jk_id" id="q_x_jk_id" value="<?php echo $t_jdkr_peg->jk_id->LookupFilterQuery(true) ?>">
<script type="text/javascript">
ft_jdkr_pegedit.CreateAutoSuggest({"id":"x_jk_id","forceSelect":true});
</script>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($t_jdkr_peg->jk_id->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_jk_id',m:0,n:10,srch:false});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" name="s_x_jk_id" id="s_x_jk_id" value="<?php echo $t_jdkr_peg->jk_id->LookupFilterQuery(false) ?>">
</span>
<?php echo $t_jdkr_peg->jk_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php
	if (in_array("t_tgl_2017", explode(",", $t_jdkr_peg->getCurrentDetailTable())) && $t_tgl_2017->DetailEdit) {
?>
...
<?php } ?>
<?php
	if (in_array("t_jk", explode(",", $t_jdkr_peg->getCurrentDetailTable())) && $t_jk->DetailEdit) {
?>
<?php if ($t_jdkr_peg->getCurrentDetailTable() <> "") { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_jdkr_peg_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php if (!isset($t_jdkr_peg_edit->Pager)) $t_jdkr_peg_edit->Pager = new cPrevNextPager($t_jdkr_peg_edit->StartRec, $t_jdkr_peg_edit->DisplayRecs, $t_jdkr_peg_edit->TotalRecs) ?>
<?php if ($t_jdkr_peg_edit->Pager->RecordCount > 0 && $t_jdkr_peg_edit->Pager->Visible) { ?>
...
<script type="text/javascript">
ft_jdkr_pegedit.Init();
</script>
<?php
$t_jdkr_peg_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_jdkr_peg_edit->Page_Terminate();
?>
