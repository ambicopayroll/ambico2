...
$t_jdkr_peg_grid = NULL; // Initialize page object first

class ct_jdkr_peg_grid extends ct_jdkr_peg {
	...
	// Page object name
	var $PageObjName = 't_jdkr_peg_grid';

	// Grid form hidden field names
	var $FormName = 'ft_jdkr_peggrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_jdkr_peg"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_pegawai_id") && $objForm->HasValue("o_pegawai_id") && $this->pegawai_id->CurrentValue <> $this->pegawai_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_tgl_id") && $objForm->HasValue("o_tgl_id") && $this->tgl_id->CurrentValue <> $this->tgl_id->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		$this->tgl_id->setOldValue($objForm->GetValue("o_tgl_id"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->jdkr_id->CurrentValue = strval($arKeys[0]); // jdkr_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
