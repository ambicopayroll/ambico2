...
$t_jk_grid = NULL; // Initialize page object first

class ct_jk_grid extends ct_jk {
	...
	// Page object name
	var $PageObjName = 't_jk_grid';

	// Grid form hidden field names
	var $FormName = 'ft_jkgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_jk"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->jk_id->CurrentValue = strval($arKeys[0]); // jk_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function EditRow() {
		...
		$conn = &$this->Connection();
		if ($this->jk_kd->CurrentValue <> "") { // Check field with unique index
			$sFilterChk = "(`jk_kd` = '" . ew_AdjustSql($this->jk_kd->CurrentValue, $this->DBID) . "')";
			...
			if ($rsChk === FALSE) {
				...
				$sIdxErrMsg = str_replace("%f", $this->jk_kd->FldCaption(), $Language->Phrase("DupIndex"));
				$sIdxErrMsg = str_replace("%v", $this->jk_kd->CurrentValue, $sIdxErrMsg);
		...
		if ($rs->EOF) {
			...
			// Check referential integrity for master table 't_jdkr_peg'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_t_jdkr_peg();
			$KeyValue = isset($rsnew['jk_id']) ? $rsnew['jk_id'] : $rsold['jk_id'];
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@jk_id@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
				$bValidMasterRecord = FALSE;
			}
			if ($bValidMasterRecord) {
				if (!isset($GLOBALS["t_jdkr_peg"])) $GLOBALS["t_jdkr_peg"] = new ct_jdkr_peg();
				$rsmaster = $GLOBALS["t_jdkr_peg"]->LoadRs($sMasterFilter);
				$bValidMasterRecord = ($rsmaster && !$rsmaster->EOF);
				$rsmaster->Close();
			}
			if (!$bValidMasterRecord) {
				$sRelatedRecordMsg = str_replace("%t", "t_jdkr_peg", $Language->Phrase("RelatedRecordRequired"));
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "t_jdkr_peg") {
				$this->jk_id->CurrentValue = $this->jk_id->getSessionValue();
			}
		if ($this->jk_kd->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(jk_kd = '" . ew_AdjustSql($this->jk_kd->CurrentValue, $this->DBID) . "')";
		...
		if ($this->jk_id->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@jk_id@", ew_AdjustSql($this->jk_id->getSessionValue(), "DB"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
