...
$kartu_scanlog_edit = NULL; // Initialize page object first

class ckartu_scanlog_edit extends ckartu_scanlog {
	...
	// Page object name
	var $PageObjName = 'kartu_scanlog_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->ks_id->CurrentValue == "") {
			$this->Page_Terminate("kartu_scanloglist.php"); // Invalid key, return to list
		}
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->ks_id->FldIsDetailKey)
			$this->ks_id->setFormValue($objForm->GetValue("x_ks_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->ks_id->CurrentValue = $this->ks_id->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// dow
			$this->dow->SetDbValueDef($rsnew, $this->dow->CurrentValue, 0, $this->dow->ReadOnly);

			// scan_tgl
			$this->scan_tgl->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->scan_tgl->CurrentValue, 0), ew_CurrentDate(), $this->scan_tgl->ReadOnly);

			// scan_jam_1
			$this->scan_jam_1->SetDbValueDef($rsnew, $this->scan_jam_1->CurrentValue, ew_CurrentTime(), $this->scan_jam_1->ReadOnly);

			// scan_jam_2
			$this->scan_jam_2->SetDbValueDef($rsnew, $this->scan_jam_2->CurrentValue, ew_CurrentTime(), $this->scan_jam_2->ReadOnly);

			// scan_jam_3
			$this->scan_jam_3->SetDbValueDef($rsnew, $this->scan_jam_3->CurrentValue, ew_CurrentTime(), $this->scan_jam_3->ReadOnly);

			// scan_jam_4
			$this->scan_jam_4->SetDbValueDef($rsnew, $this->scan_jam_4->CurrentValue, ew_CurrentTime(), $this->scan_jam_4->ReadOnly);
...
// Create page object
if (!isset($kartu_scanlog_edit)) $kartu_scanlog_edit = new ckartu_scanlog_edit();

// Page init
$kartu_scanlog_edit->Page_Init();

// Page main
$kartu_scanlog_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kartu_scanlog_edit->Page_Render();
?>
...
var CurrentForm = fkartu_scanlogedit = new ew_Form("fkartu_scanlogedit", "edit");

// Validate form
fkartu_scanlogedit.Validate = function() {
...
fkartu_scanlogedit.Form_CustomValidate = 
...
fkartu_scanlogedit.ValidateRequired = true;
<?php } else { ?>
fkartu_scanlogedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$kartu_scanlog_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $kartu_scanlog_edit->ShowPageHeader(); ?>
<?php
$kartu_scanlog_edit->ShowMessage();
?>
<form name="fkartu_scanlogedit" id="fkartu_scanlogedit" class="<?php echo $kartu_scanlog_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kartu_scanlog_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kartu_scanlog_edit->Token ?>">
<?php } ?>
...
<?php if ($kartu_scanlog->ks_id->Visible) { // ks_id ?>
	<div id="r_ks_id" class="form-group">
		<label id="elh_kartu_scanlog_ks_id" class="col-sm-2 control-label ewLabel"><?php echo $kartu_scanlog->ks_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $kartu_scanlog->ks_id->CellAttributes() ?>>
<span id="el_kartu_scanlog_ks_id">
<span<?php echo $kartu_scanlog->ks_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $kartu_scanlog->ks_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="kartu_scanlog" data-field="x_ks_id" name="x_ks_id" id="x_ks_id" value="<?php echo ew_HtmlEncode($kartu_scanlog->ks_id->CurrentValue) ?>">
<?php echo $kartu_scanlog->ks_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kartu_scanlog_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fkartu_scanlogedit.Init();
</script>
<?php
$kartu_scanlog_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$kartu_scanlog_edit->Page_Terminate();
?>
